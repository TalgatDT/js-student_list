(() => {
	function addStudent() {
		add_btn = document.getElementById('form__btn')
		add_btn.addEventListener('click', function () {

			let add__form = document.getElementById('add__form')
			if (document.getElementById('alertError')) {
				add__form.removeChild(document.getElementById('alertError'))
			}
			let valid_form = validateForm(add__form)
			if (valid_form != false) {
				let data = converData(add__form)
				add__form.reset()
	...
	function validateForm(student_form) {
		valid = true;
		let form = document.getElementById('add__form')
	...
	function converData(student_form) {
		let name = student_form.querySelector('#addName').value.trim()
		let surname = student_form.querySelector('#addSurname').value.trim()
		let patronymic = student_form.querySelector('#addPatronymic').value.trim()
		let birth_date = student_form.querySelector('#addBirthDate').value.trim()
		let study_start = parseInt(student_form.querySelector('#addStartYear').value.trim())
		let faculty = student_form.querySelector('#addFaculty').value.trim()
		...
		if (data) {
			for (let item of data) {
				if (item.hasOwnProperty('fullname') && item.name != '') {
					let table = document.getElementById('search__table_body')
